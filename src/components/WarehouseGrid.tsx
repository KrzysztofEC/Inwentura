...
export function WarehouseGrid({ cfg, cells }: { cfg: WarehouseConfig; cells: Cell[] }) {
  ...
  const selStartRef = useRef<{ col: string; rowIdx: number } | null>(null);
  const selCurrentRef = useRef<{ col: string; rowIdx: number } | null>(null);
  ...
  function getSelection(
    start: { col: string; rowIdx: number },
    end: { col: string; rowIdx: number }
  ): Set<string> {
    const c1 = editableCols.indexOf(start.col);
    const c2 = editableCols.indexOf(end.col);
    const r1 = start.rowIdx;
    const r2 = end.rowIdx;
  ...
  // Globalne mouse events dla drag
  useEffect(() => {
    function onMouseUp() {
    ...
    function onMouseMove(e: MouseEvent) {
      ...
      // Znajdź element pod kursorem z data-cell-key
      ...
      const rowIdx = numericRows.indexOf(parseInt(rowStr));
      if (rowIdx === -1) return;
      const cur = { col, rowIdx };
      selCurrentRef.current = cur;
      if (selStartRef.current) {
        setSelected(getSelection(selStartRef.current, cur));
      }
    }
  ...
  }, [isSelecting, numericRows, editableCols]);

  function startSelection(col: string, row: number, e: React.MouseEvent) {
    if (isRoadCol(col)) return;
    e.preventDefault();
    const rowIdx = numericRows.indexOf(row);
    if (rowIdx === -1) return;
    const start = { col, rowIdx };
    selStartRef.current = start;
    selCurrentRef.current = start;
    setIsSelecting(true);
  ...
  function handleKey(e: React.KeyboardEvent<HTMLInputElement>, col: string, row: number, field: Field) {
    ...
    function rowDown(r: number): number | null { if (cfg.rowsReversed) return r > 1 ? r - 1 : null; return r < rows ? r + 1 : null; }
    function rowUp(r: number): number | null { if (cfg.rowsReversed) return r < rows ? r + 1 : null; return r > 1 ? r - 1 : null; }
  ...
  function colWidth(col: string): number { return isRoadCol(col) ? 45 : 70; }
  ...
  // Renderuj komórkę z nakładką do zaznaczania
  function renderTd(col: string, row: number, colSpan: number, tdKey: string, field: Field) {
    ...
    return (
      <td key={tdKey} colSpan={colSpan}
        data-cell-key={road ? undefined : key}
        className={`border ${border} p-0 align-middle ${bg} relative`}>
        {renderInput(col, row, field)}
        {/* Nakładka do zaznaczania — widoczna tylko gdy jest zaznaczenie LUB podczas przeciągania */}
        {!road && (
          <div
            className="absolute inset-0 z-20"
            style={{ cursor: isSelecting ? 'crosshair' : 'default', opacity: 0 }}
            onMouseDown={(e) => { e.preventDefault(); startSelection(col, row, e); setSelected(new Set()); }}
          />
        )}
  ...
  return (
    <div className="bg-white rounded shadow-sm overflow-x-auto">
      ...
      {selected.size === 0 && (
        <div className="px-2 py-0.5 text-[10px] text-gray-400 bg-gray-50 border-b">
          💡 Aby zaznaczyć komórki: przytrzymaj <strong>Ctrl</strong> i kliknij myszką, lub kliknij prawym przyciskiem i przeciągnij
        </div>
      )}
            ...
            if (rNum === 'M') {
              return (
                <tr key="magazynek">
                  ...
                  <td colSpan={allCols.length * 2} className="border bg-amber-50 text-xs italic text-gray-500 px-2">(rząd specjalny)</td>
                  <th className="bg-gray-900 text-white w-9 text-center align-middle text-xs">M</th>
                </tr>
              );
            }
      ...
      <div className="text-xs text-gray-500 px-2 py-1.5 border-t bg-gray-50">
        ...
        <strong> Ctrl+klik</strong> na komórkę = zaznacz, potem przeciągnij · <strong>Delete</strong> = usuń zaznaczone · <strong>Esc</strong> = odznacz.
      </div>
    </div>
  );
}
...
function SaveBar({ states, saveStatus, flushAll, selected, onDeleteSelected, onClearSelected }: {
  ...
  selected: Set<string>; onDeleteSelected: () => void; onClearSelected: () => void;
}) {
